			...
			if (!strncmp(p->name, d->__nextchar, namelen))
			{
					...
					|| pfound->val != p->val)
				{
					if (!ambig_fallback)
					{
						...
						{
							if ((ambig_set = malloc(n_options * sizeof(char))) == NULL)
								ambig_fallback = 1;
...
int _getopt_internal_r_a(int argc, char* const* argv, const char* optstring, const struct option_a* longopts, int* longind, int long_only, struct _getopt_data_a* d, int posixly_correct)
{
	...
	if (d->__nextchar == NULL || *d->__nextchar == '\0')
	{
		...
		if (longopts)
		{
			if (argv[d->optind][1] == '-')
			{
				...
				return process_long_option_a(argc, (char**)argv, optstring, longopts, longind, long_only, d, print_errors, "--");
			}
				...
				|| !strchr(optstring, argv[d->optind][1])))
			{
				...
				code = process_long_option_a(argc, (char**)argv, optstring, longopts,
					longind, long_only, d,
					print_errors, "-");
				if (code != -1)
					return code;
	...
	}
	{
		...
		if (temp[0] == 'W' && temp[1] == ';' && longopts != NULL)
		{
			...
			d->optarg = NULL;
			return process_long_option_a(argc, (char**)argv, optstring, longopts, longind, 0, d, print_errors, "-W ");
		}
		if (temp[1] == ':')
		{
			if (temp[2] == ':')
...
static int process_long_option_w(int argc, wchar_t** argv, const wchar_t* optstring, const struct option_w* longopts, int* longind, int long_only, struct _getopt_data_w* d, int print_errors, const wchar_t* prefix)
{
	...
	if (pfound == NULL)
	{
			...
			if (!wcsncmp(p->name, d->__nextchar, namelen))
			{
					...
					|| pfound->val != p->val)
				{
					if (!ambig_fallback)
					{
						...
						{
							if ((ambig_set = malloc(n_options * sizeof(wchar_t))) == NULL)
								ambig_fallback = 1;
		...
		if (ambig_set || ambig_fallback)
		{
			...
			{
				if (ambig_fallback)
					fwprintf(stderr, "%s: option '%s%s' is ambiguous\n", argv[0], prefix, d->__nextchar);
				else
				{
					_lock_file(stderr);
					fwprintf(stderr, "%s: option '%s%s' is ambiguous; possibilities:", argv[0], prefix, d->__nextchar);
					for (option_index = 0; option_index < n_options; option_index++)
						if (ambig_set[option_index])
							fwprintf(stderr, " '%s%s'", prefix, longopts[option_index].name);
					fwprintf(stderr, "\n");
					_unlock_file(stderr);
				}
			}
	...
	if (pfound == NULL)
	{
		...
		{
			if (print_errors)
				fwprintf(stderr, "%s: unrecognized option '%s%s'\n", argv[0], prefix, d->__nextchar);
			d->__nextchar = NULL;
			d->optind++;
			d->optopt = 0;
			return L'?';
	...
	if (*nameend)
	{
		...
		{
			if (print_errors)
				fwprintf(stderr, "%s: option '%s%s' doesn't allow an argument\n", argv[0], prefix, pfound->name);
			d->optopt = pfound->val;
			return L'?';
		}
	}
	else if (pfound->has_arg == 1)
	{
		...
		{
			if (print_errors)
				fwprintf(stderr, "%s: option '%s%s' requires an argument\n", argv[0], prefix, pfound->name);
			d->optopt = pfound->val;
...
static const wchar_t* _getopt_initialize_w(const wchar_t* optstring, struct _getopt_data_w* d, int posixly_correct)
{
	...
	else if (posixly_correct | !!_wgetenv("POSIXLY_CORRECT"))
		d->__ordering = REQUIRE_ORDER;
	else
		d->__ordering = PERMUTE;
...
#define NONOPTION_P (argv[d->optind][0] != L'-' || argv[d->optind][1] == L'\0')
	...
	if (d->__nextchar == NULL || *d->__nextchar == L'\0')
	{
		...
		if (d->optind != argc && !wcscmp(argv[d->optind], "--"))
		{
			d->optind++;
			if (d->__first_nonopt != d->__last_nonopt
				&& d->__last_nonopt != d->optind)
		...
		if (longopts)
		{
			...
			{
				d->__nextchar = argv[d->optind] + 2;
				return process_long_option_w(argc, (wchar_t**)argv, optstring, longopts, longind, long_only, d, print_errors, "--");
			}
			if (long_only && (argv[d->optind][2] || !wcschr(optstring, argv[d->optind][1])))
			{
				int code;
				d->__nextchar = argv[d->optind] + 1;
				code = process_long_option_w(argc, (wchar_t**)argv, optstring, longopts, longind, long_only, d, print_errors, "-");
				if (code != -1)
					return code;
			}
		}
	...
	}
	{
		...
		{
			if (print_errors)
				fwprintf(stderr, "%s: invalid option -- '%c'\n", argv[0], c);
			d->optopt = c;
			return L'?';
		}
		if (temp[0] == L'W' && temp[1] == L';' && longopts != NULL)
		{
			...
			{
				if (print_errors)
					fwprintf(stderr, "%s: option requires an argument -- '%c'\n", argv[0], c);
				d->optopt = c;
				if (optstring[0] == L':')
					c = L':';
				else
			...
			d->__nextchar = d->optarg;
			d->optarg = NULL;
			return process_long_option_w(argc, (wchar_t**)argv, optstring, longopts, longind,
				0, d, print_errors, "-W ");
		}
		if (temp[1] == L':')
		{
